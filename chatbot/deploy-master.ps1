...
function Invoke-AzSafe {
    ...
    try {
        ...
        if ($ShowErrors) {
            $output = & az @ArgsArray 2>&1
        } else {
            $null = & az @ArgsArray 2>&1
        }
...
function Deploy-Azure {
    ...
    if (-not $diagnostics.AppNameAvailable) {
        ...
        Write-Warning "App name '$AppName' is already taken!"
        Write-Info "Conflicting app found in resource group: $($conflictApp.resourceGroup)"
        Write-Info "App URL: $($conflictApp.defaultHostName)"
        
        if ($ForceCleanup) {
            Write-Info "ForceCleanup flag set - will delete conflicting resources..."
            if ($conflictApp.resourceGroup -ne $ResourceGroup) {
                Write-Warning "Conflicting app is in different resource group: $($conflictApp.resourceGroup)"
                ...
                Remove-AzureWebApp -AppName $AppName -ResourceGroup $ResourceGroup
            }
            Start-Sleep -Seconds 5  # Wait for deletion to propagate
        } else {
            Write-Error "App name conflict detected. Use -ForceCleanup to automatically delete conflicting resources."
    ...
    if (-not (Invoke-AzSafe -ArgsArray $configSetStartupArgs -OperationName "set startup command" -Context $context)) {
        exit 1
    }
    ...
    Get-Content ".env" -Raw | ForEach-Object {
        ...
        foreach ($line in $lines) {
            ...
            if ($line -match '^([^#=]+?)=(.*)$') {
                ...
                if ($key -and $value) {
                    # Track if GROQ_API_KEY is found
    ...
    if ([string]::IsNullOrWhiteSpace($envVars["GROQ_API_KEY"])) {
        Write-Error "GROQ_API_KEY is empty in .env file!"
        Write-Error "Please set a valid Groq API key in your .env file."
        exit 1
    }
    ...
    if (-not (Invoke-AzSafe -ArgsArray $deployZipArgs -OperationName "deploy application" -Context $context)) {
    ...
    Write-Info "Waiting for app to start (45 seconds for first deployment)..."
    ...
    for ($i = 1; $i -le $maxRetries; $i++) {
        try {
            $response = Invoke-RestMethod -Uri "$appUrl/healthz" -TimeoutSec 15
            Write-Success "Health check passed: $($response | ConvertTo-Json)"
            $healthCheckPassed = $true
            break
        } catch {
            if ($i -lt $maxRetries) {
                Write-Warning "Health check attempt $i failed. Retrying in 15 seconds..."
                Start-Sleep -Seconds 15
            } else {
