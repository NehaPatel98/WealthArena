...
app.include_router(knowledge_router, prefix="", tags=["knowledge"])  # Routes already have /v1 prefix
...
# Add /search alias (without /v1 prefix) for backward compatibility
@app.get("/search")
async def search_alias(
    q: str = Query(..., description="Search query"),
    k: int = Query(5, description="Number of results to return", ge=1, le=20)
):
    """Search endpoint alias at root level (also available at /v1/search)"""
