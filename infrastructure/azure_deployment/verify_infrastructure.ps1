    ...
    try {
        ...
        if ($keyVault -and $keyVault.name) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ColorOutput "      Fix with: .\fix_keyvault_permissions.ps1 -KeyVault $keyVaultName" $Blue
                $allChecksPassed = $false
            }
        } else {
...
if ($allChecksPassed) {
    ...
    Write-ColorOutput "     - Fix Key Vault permissions: .\fix_keyvault_permissions.ps1" $Blue
