...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ColorOutput "To fix, run: .\fix_keyvault_permissions.ps1 -KeyVault $KeyVault" $Blue
    }
} catch {
...
if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($dbPasswordOutput) -and -not $dbPasswordOutput.StartsWith("az:")) {
    ...
    Write-ColorOutput "  1. Run the fix script: .\fix_keyvault_permissions.ps1 -KeyVault $KeyVault" $Blue
