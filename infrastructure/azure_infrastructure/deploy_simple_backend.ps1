# WealthArena Simple Backend Deployment
# This script deploys the backend code to Azure App Service using zip deployment
...
function Deploy-BackendAPI {
    Write-ColorOutput "Deploying Backend API to Azure App Service..." $Blue
    
    try {
        $backendAppName = "wealtharena-backend-$Environment"
        
        # Create a simple Flask app for Azure
        $flaskApp = @"
...
"@
        ...
        # Save Flask app
        $flaskApp | Out-File -FilePath "app.py" -Encoding UTF8
...
<configuration>
  <system.webServer>
    ...
    <httpPlatform processPath="D:\home\Python310\python.exe" arguments="D:\home\site\wwwroot\app.py" stdoutLogEnabled="true" stdoutLogFile="D:\home\LogFiles\python.log" startupTimeLimit="60" startupRetryCount="3">
...
"@
        ...
        Compress-Archive -Path "app.py", "requirements.txt", "web.config" -DestinationPath "backend-deployment.zip" -Force
        
        # Deploy to Azure
        Write-ColorOutput "Deploying to Azure App Service..." $Blue
        az webapp deployment source config-zip --resource-group $ResourceGroupName --name $backendAppName --src "backend-deployment.zip"
        ...
        # Clean up
        if (Test-Path "app.py") { Remove-Item "app.py" -Force }
        if (Test-Path "requirements.txt") { Remove-Item "requirements.txt" -Force }
        if (Test-Path "web.config") { Remove-Item "web.config" -Force }
        if (Test-Path "backend-deployment.zip") { Remove-Item "backend-deployment.zip" -Force }
    }
}

function Deploy-Chatbot {
    Write-ColorOutput "Deploying RAG Chatbot to Azure App Service..." $Blue
    
    try {
        $chatbotAppName = "wealtharena-chatbot-$Environment"
        
        # Create a simple Flask chatbot app
        $chatbotApp = @"
from flask import Flask, request, jsonify
import json
import uuid
...
<configuration>
  <system.webServer>
    ...
    <httpPlatform processPath="D:\home\Python310\python.exe" arguments="D:\home\site\wwwroot\chatbot_app.py" stdoutLogEnabled="true" stdoutLogFile="D:\home\LogFiles\python.log" startupTimeLimit="60" startupRetryCount="3">
      <environmentVariables>
        <environmentVariable name="PORT" value="8000"/>
      </environmentVariables>
    </httpPlatform>
...
"@
        $webConfig | Out-File -FilePath "chatbot_web.config" -Encoding UTF8
        
        # Deploy using zip
        Write-ColorOutput "Creating chatbot deployment package..." $Blue
        Compress-Archive -Path "chatbot_app.py", "chatbot_requirements.txt", "chatbot_web.config" -DestinationPath "chatbot-deployment.zip" -Force
        
        # Deploy to Azure
        Write-ColorOutput "Deploying chatbot to Azure App Service..." $Blue
        az webapp deployment source config-zip --resource-group $ResourceGroupName --name $chatbotAppName --src "chatbot-deployment.zip"
        ...
        if (Test-Path "chatbot_web.config") { Remove-Item "chatbot_web.config" -Force }
        if (Test-Path "chatbot-deployment.zip") { Remove-Item "chatbot-deployment.zip" -Force }
    }
}
...
# Main execution
Write-ColorOutput "WealthArena Simple Backend Deployment" $Blue
...
# Deploy chatbot
$chatbotDeployed = Deploy-Chatbot
...
Write-ColorOutput "1. Test the mobile app with these URLs" $Blue
Write-ColorOutput "2. Verify user registration and login" $Blue
Write-ColorOutput "3. Test trading signals and portfolio features" $Blue
Write-ColorOutput "4. Test chatbot integration" $Blue

Write-ColorOutput "" $Blue
Write-ColorOutput "Simple backend deployment complete!" $Blue
