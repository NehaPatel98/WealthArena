...
rl_cfg = cfg["services"]["rl"]
model_path = rl_cfg["model_path"]
model_name = rl_cfg["model_name"]
service_name = rl_cfg["service_name"]
entry_script = rl_cfg["entry_script"]
pip_packages = rl_cfg.get("pip_packages", ["azureml-defaults"]) 
...
# Ensure model path exists (allow placeholder directory)
model_path_abs = os.path.abspath(model_path)
if not os.path.exists(model_path_abs):
    os.makedirs(model_path_abs, exist_ok=True)
    # create a placeholder artifact so registration succeeds
...
# Environment
env = Environment('wealtharena-rl-env')
