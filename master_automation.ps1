...
param(
    [string]$ConfigFile = "automation_config.yaml",
...
$commonScript = Join-Path $script:ScriptDir "automation_common.ps1"
    ...
    try {
        ...
        try {
            ...
            if ($errorMessage -match "SQL|database|getaddrinfo") {
                ...
                Write-ColorOutput "Run: .\diagnose_database_connectivity.ps1 -ResourceGroup $rg" $Blue
            } elseif ($errorMessage -match "Key Vault|secret|permission") {
                Write-ColorOutput "Key Vault access issue detected." $Yellow
                Write-ColorOutput "Run: .\fix_keyvault_permissions.ps1 -ResourceGroup $rg" $Blue
...
function Initialize-AutomationEnvironment {
    ...
    $configFile = "automation_config.yaml"
    $configExampleFile = "automation_config.example.yaml"
