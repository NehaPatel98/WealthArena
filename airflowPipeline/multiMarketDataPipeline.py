...
def _build_sql_uri_from_conn(conn_id: str) -> str:
    ...
    # IMPORTANT: empty-string fallback avoids the SyntaxError you saw earlier.
    return (f"mssql://{c.login}:{c.password}@{c.host}:{c.port or 1433}/{c.schema or ''}"
...
def _run_script(pyfile: str, extra_env: dict | None = None):
    ...
    p = subprocess.run(["python", full_path], cwd=PIPELINE_DIR, env=env, capture_output=True, text=True)
...
CRON_UTC_FOR_2AM_SYDNEY = "0 15 * * *"   # 15:00 UTC ≈ 02:00 Sydney (AEDT)
...
with DAG(
    ...
    schedule_interval=CRON_UTC_FOR_2AM_SYDNEY,
    start_date=pendulum.datetime(2025, 11, 13, 15, 0, tz=UTC),
    catchup=False,
    max_active_runs=1,
