...
class MLflowTracker:
    ...
    def _setup_mlflow(self):
        ...
        try:
            ...
            # If server connection fails, fallback to local file storage
            if self.tracking_uri.startswith("http://") or self.tracking_uri.startswith("https://"):
                logger.warning(f"MLflow server at {self.tracking_uri} is not available: {e}")
                ...
                try:
                    ...
                    if experiment is None:
                        ...
                        logger.info(f"Created new local experiment: {self.experiment_name}")
                    else:
                        experiment_id = experiment.experiment_id
                        logger.info(f"Using existing local experiment: {self.experiment_name}")
                    self.experiment_id = experiment_id
                except Exception as local_error:
                    logger.error(f"Error setting up local MLflow experiment: {local_error}")
