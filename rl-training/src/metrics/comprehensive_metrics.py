...
class MetricsCollector:
    ...
    def export_progress_report_metrics(self, output_path: str = "PROGRESS_REPORT_METRICS.md"):
        """
        Export metrics in progress report format
        
        Args:
            output_path: Path to save the metrics report
        """
        summary = self.get_metrics_summary(hours=24)
        ...
        if "data" in summary:
            data = summary["data"]
            report_lines.extend([
                f"| Avg Query Latency (ms) | {data.get('avg_query_latency', 0) * 1000:.2f} |",
                f"| P95 Query Latency (ms) | {data.get('p95_query_latency', 0) * 1000:.2f} |",
                f"| P99 Query Latency (ms) | {data.get('p99_query_latency', 0) * 1000:.2f} |",
                f"| Avg Disk Usage % | {data.get('avg_disk_usage', 0):.2f}% |"
            ])
        ...
        if "backend_api" in summary:
            backend = summary["backend_api"]
            report_lines.extend([
                f"| Response Time (ms) | {backend.get('avg_response_time', 0) * 1000:.2f} |",
                f"| Error Rate | {backend.get('avg_error_rate', 0):.4f} |",
                f"| CPU % | {backend.get('avg_cpu_utilization', 0):.2f}% |",
                f"| Memory % | {backend.get('avg_memory_utilization', 0):.2f}% |"
            ])
        
        if "pipeline" in summary:
            pipeline = summary["pipeline"]
            report_lines.extend([
                f"| Avg Success Rate | {pipeline.get('avg_success_rate', 0):.2f}% |",
                f"| Avg Failure Rate | {pipeline.get('avg_failure_rate', 0):.4f} |"
            ])
        ...
        if "code" in summary:
            code = summary["code"]
            report_lines.extend([
                f"| Cyclomatic Complexity | {code.get('avg_cyclomatic_complexity', 0):.2f} |",
                f"| Duplication Rate | {code.get('avg_duplication_rate', 0):.4f} |",
                f"| Technical Debt (hours) | {code.get('avg_technical_debt', 0):.2f} |"
            ])
        ...
        if "ml" in summary:
            ml = summary["ml"]
            report_lines.extend([
                f"| Inference Time (ms) | {ml.get('avg_inference_time', 0) * 1000:.2f} |",
                f"| RMSE | {ml.get('avg_rmse', 0):.4f} |",
                f"| AUC | {ml.get('avg_auc', 0):.4f} |",
                f"| F1 Score | {ml.get('avg_f1_score', 0):.4f} |"
            ])
        ...
        if "testing" in summary:
            ...
            report_lines.extend([
                f"| Avg Coverage % | {testing.get('avg_coverage', 0):.2f}% |",
                f"| Avg Failure Rate | {testing.get('avg_failure_rate', 0):.4f} |"
            ])
        ...
        logger.info(f"Progress report metrics exported to {output_path}")
        ...
        logger.info(f"Metrics JSON exported to {json_path}")
