"""
ASX Stocks Training and Evaluation System

This module provides specialized training and evaluation for the ASX Stocks agent
with comprehensive metrics and backtesting capabilities.
"""

import numpy as np
import pandas as pd
from typing import Dict, Any, List, Tuple, Optional
import logging
from datetime import datetime, timedelta
from pathlib import Path
import yaml
import json
from dataclasses import dataclass

from ..models.specialized_agent_factory import SpecializedAgentFactory, AssetType
from ..data.asx.asx_symbols import get_asx_200_symbols, get_symbols_by_sector
from ..environments.base_trading_env import BaseTradingEnv, TradingEnvConfig
from ..environments.trading_env import WealthArenaTradingEnv
from ..data.benchmarks.benchmark_data import BenchmarkDataFetcher, BenchmarkConfig
from .model_checkpoint import ModelCheckpoint, create_mock_model_state, create_mock_training_state, create_mock_optimizer_state
from .train_agents import AdvancedTradingTrainer
from .callbacks import WinRateCallback
from ..utils.data_loader import load_processed_data, prepare_training_data
import ray
from ray.rllib.algorithms.ppo import PPO, PPOConfig
import torch

logger = logging.getLogger(__name__)


@dataclass
class ASXStocksConfig:
    """Configuration for ASX Stocks training"""
    # Training parameters
    start_date: str = "2015-01-01"
    end_date: str = "2025-09-26"
    validation_split: float = 0.2
    test_split: float = 0.2
    
    # ASX specific
    symbols: List[str] = None
    lookback_window: int = 30
    episode_length: int = 252
    
    # Training parameters
    learning_rate: float = 3e-4
    batch_size: int = 4000
    gamma: float = 0.99
    lambda_: float = 0.95
    epochs: int = 1000
    
    # Risk management
    initial_cash: float = 1_000_000.0
    max_position_size: float = 0.15
    max_portfolio_risk: float = 0.12
    transaction_cost_rate: float = 0.001  # Higher for stocks
    
    def __post_init__(self):
        if self.symbols is None:
            self.symbols = get_asx_200_symbols()[:50]  # Top 50 ASX stocks


class ASXStocksEvaluator:
    """Comprehensive evaluation system for ASX Stocks agent"""
    
    def __init__(self, config: ASXStocksConfig):
        self.config = config
        self.metrics = {}
        
    def calculate_returns(self, portfolio_values: List[float]) -> np.ndarray:
        """Calculate portfolio returns"""
        if len(portfolio_values) < 2:
            return np.array([])
        return np.diff(portfolio_values) / portfolio_values[:-1]
    
    def calculate_sharpe_ratio(self, returns: np.ndarray, risk_free_rate: float = 0.02) -> float:
        """Calculate Sharpe ratio"""
        if len(returns) == 0 or np.std(returns) == 0:
            return 0.0
        excess_returns = returns - risk_free_rate / 252
        return np.mean(excess_returns) / np.std(excess_returns) * np.sqrt(252)
    
    def calculate_max_drawdown(self, portfolio_values: List[float]) -> float:
        """Calculate maximum drawdown"""
        if len(portfolio_values) < 2:
            return 0.0
        peak = np.maximum.accumulate(portfolio_values)
        drawdown = (np.array(portfolio_values) - peak) / peak
        return np.min(drawdown)
    
    def evaluate_performance(self, portfolio_values: List[float], returns: np.ndarray, 
                           positions_history: List[Dict], benchmark_returns: np.ndarray = None) -> Dict[str, Any]:
        """Comprehensive performance evaluation"""
        
        # Basic metrics
        total_return = (portfolio_values[-1] - portfolio_values[0]) / portfolio_values[0]
        annual_return = (1 + total_return) ** (252 / len(portfolio_values)) - 1
        volatility = np.std(returns) * np.sqrt(252)
        
        # Risk metrics
        max_drawdown = self.calculate_max_drawdown(portfolio_values)
        sharpe_ratio = self.calculate_sharpe_ratio(returns)
        
        # Trading metrics
        win_rate = np.sum(returns > 0) / len(returns) if len(returns) > 0 else 0
        profit_factor = self._calculate_profit_factor(returns)
        
        # Benchmark comparison
        benchmark_metrics = {}
        if benchmark_returns is not None and len(benchmark_returns) > 0:
            benchmark_sharpe = self.calculate_sharpe_ratio(benchmark_returns)
            benchmark_return = np.mean(benchmark_returns) * 252
            benchmark_metrics = {
                "benchmark_return": benchmark_return,
                "benchmark_sharpe": benchmark_sharpe,
                "excess_return": annual_return - benchmark_return,
                "alpha": annual_return - benchmark_return,
                "beta": np.cov(returns, benchmark_returns)[0, 1] / np.var(benchmark_returns) if np.var(benchmark_returns) > 0 else 0
            }
        
        return {
            "returns": {
                "total_return": total_return,
                "annual_return": annual_return,
                "volatility": volatility
            },
            "risk": {
                "max_drawdown": max_drawdown,
                "sharpe_ratio": sharpe_ratio
            },
            "trading": {
                "win_rate": win_rate,
                "profit_factor": profit_factor
            },
            "benchmark": benchmark_metrics
        }
    
    def _calculate_profit_factor(self, returns: np.ndarray) -> float:
        """Calculate profit factor"""
        if len(returns) == 0:
            return 0.0
        positive_returns = returns[returns > 0]
        negative_returns = returns[returns < 0]
        if len(negative_returns) == 0:
            return float('inf') if len(positive_returns) > 0 else 0.0
        gross_profit = np.sum(positive_returns)
        gross_loss = abs(np.sum(negative_returns))
        return gross_profit / gross_loss if gross_loss > 0 else 0.0


class ASXStocksTrainer:
    """Specialized trainer for ASX Stocks agent"""
    
    def __init__(self, config: ASXStocksConfig):
        self.config = config
        self.evaluator = ASXStocksEvaluator(config)
        
        # Create agent configuration
        self.agent_config = SpecializedAgentFactory.create_agent_config(
            AssetType.ASX_STOCKS,
            num_assets=len(config.symbols),
            symbols=config.symbols,
            episode_length=config.episode_length,
            lookback_window_size=config.lookback_window
        )
        
        # Initialize model checkpoint manager
        self.checkpoint_manager = ModelCheckpoint("checkpoints/asx_stocks")
        
        # Initialize benchmark data fetcher
        benchmark_config = BenchmarkConfig(
            start_date=config.start_date,
            end_date=config.end_date
        )
        self.benchmark_fetcher = BenchmarkDataFetcher(benchmark_config)
        
        # Training results
        self.training_results = {}
        self.evaluation_results = {}
        
        logger.info(f"ASX Stocks Trainer initialized with {len(config.symbols)} stocks")
    
    def generate_synthetic_data(self, num_days: int = 1000) -> pd.DataFrame:
        """Generate synthetic ASX stocks data for training"""
        logger.info("Generating synthetic ASX stocks data...")
        
        np.random.seed(42)
        dates = pd.date_range(start=self.config.start_date, end=self.config.end_date, freq='D')
        
        # Generate correlated returns for ASX stocks
        n_stocks = len(self.config.symbols)
        correlation_matrix = self._generate_stock_correlation_matrix(n_stocks)
        
        returns = np.random.multivariate_normal(
            mean=np.full(n_stocks, 0.0008),  # 0.08% daily return for stocks
            cov=correlation_matrix * 0.0004,  # 2% daily volatility
            size=len(dates)
        )
        
        # Generate OHLCV data
        data = {}
        for i, symbol in enumerate(self.config.symbols):
            stock_returns = returns[:, i]
            prices = 100.0 * np.exp(np.cumsum(stock_returns))
            
            # Generate OHLCV
            open_prices = prices * (1 + np.random.normal(0, 0.001, len(prices)))
            close_prices = prices
            high_prices = np.maximum(open_prices, close_prices) * (1 + np.abs(np.random.normal(0, 0.005, len(prices))))
            low_prices = np.minimum(open_prices, close_prices) * (1 - np.abs(np.random.normal(0, 0.005, len(prices))))
            volumes = np.random.lognormal(8, 0.5, len(prices))
            
            data[symbol] = pd.DataFrame({
                'Open': open_prices,
                'High': high_prices,
                'Low': low_prices,
                'Close': close_prices,
                'Volume': volumes
            }, index=dates)
        
        # Create multi-level DataFrame
        multi_level_data = {}
        for symbol, df in data.items():
            for col in df.columns:
                multi_level_data[(symbol, col)] = df[col]
        
        return pd.DataFrame(multi_level_data)
    
    def _generate_stock_correlation_matrix(self, n_stocks: int) -> np.ndarray:
        """Generate realistic correlation matrix for ASX stocks"""
        base_corr = 0.3
        correlation_matrix = np.full((n_stocks, n_stocks), base_corr)
        np.fill_diagonal(correlation_matrix, 1.0)
        
        # Add sector-like correlations
        sector_size = n_stocks // 5
        for i in range(0, n_stocks, sector_size):
            end_idx = min(i + sector_size, n_stocks)
            sector_corr = 0.6
            correlation_matrix[i:end_idx, i:end_idx] = sector_corr
            np.fill_diagonal(correlation_matrix[i:end_idx, i:end_idx], 1.0)
        
        return correlation_matrix
    
    def load_real_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """Load real market data and split into train/val/test"""
        logger.info(f"Loading real data for {len(self.config.symbols)} symbols...")
        
        # Map asset class to data directory
        asset_class_map = {
            'stocks': 'stocks',
            'asx_stocks': 'stocks'
        }
        
        asset_class = asset_class_map.get('asx_stocks', 'stocks')
        
        # Load data
        full_data = load_processed_data(
            asset_class=asset_class,
            symbols=self.config.symbols,
            start_date=self.config.start_date,
            end_date=self.config.end_date
        )
        
        if full_data.empty:
            logger.warning("No real data loaded, falling back to synthetic")
            return None, None, None
        
        # Temporal split (80% train, 10% val, 10% test)
        n = len(full_data)
        train_end = int(n * 0.8)
        val_end = int(n * 0.9)
        
        train_data = full_data.iloc[:train_end]
        val_data = full_data.iloc[train_end:val_end]
        test_data = full_data.iloc[val_end:]
        
        logger.info(f"Loaded {n} days of data: train={len(train_data)}, val={len(val_data)}, test={len(test_data)}")
        
        return train_data, val_data, test_data
    
    def calculate_win_rate(self, trades: List[Dict]) -> float:
        """Calculate win rate from trade history"""
        if not trades:
            return 0.0
        
        winning_trades = sum(1 for t in trades if t.get('pnl', 0) > 0)
        return winning_trades / len(trades) if len(trades) > 0 else 0.0
    
    def train_agent(self, training_config: Dict[str, Any] = None) -> Dict[str, Any]:
        """Train the ASX Stocks agent using real Ray RLlib training"""
        logger.info("Starting ASX Stocks agent training...")
        
        # Load real data
        train_data, val_data, test_data = self.load_real_data()
        
        if train_data is None:
            logger.warning("No real data available, using synthetic data")
            train_data = self.generate_synthetic_data(1000)
            val_data = None
            test_data = None
        
        try:
            # Initialize Ray if not already initialized
            if not ray.is_initialized():
                ray.init(local_mode=False, num_cpus=8, ignore_reinit_error=True)
            
            # Create environment config
            env_config = {
                "num_assets": len(self.config.symbols),
                "symbols": self.config.symbols,
                "initial_cash": self.config.initial_cash,
                "episode_length": self.config.episode_length,
                "lookback_window_size": self.config.lookback_window,
                "transaction_cost_rate": self.config.transaction_cost_rate,
                "use_real_data": True,
                "data_path": "data/processed",
                "asset_class": "stocks",
                "start_date": self.config.start_date,
                "end_date": self.config.end_date,
                "market_data": train_data,
                "reward_weights": training_config.get("environment", {}).get("reward_weights", {}) if training_config else {}
            }
            
            # Create PPO algorithm config
            if training_config:
                algo_config = self._create_ppo_config_from_yaml(training_config)
            else:
                algo_config = PPOConfig().environment(env=WealthArenaTradingEnv, env_config=env_config)
                algo_config.training(
                    lr=3e-4,
                    gamma=0.99,
                    lambda_=0.95,  # Changed from gae_lambda to lambda_
                    clip_param=0.2,
                    entropy_coeff=0.01,
                    vf_loss_coeff=0.5,
                    train_batch_size=4000,
                    minibatch_size=128,  # Changed from sgd_minibatch_size to minibatch_size
                    num_sgd_iter=10
                )
                algo_config.env_runners(num_env_runners=4, num_envs_per_env_runner=2)
                algo_config.callbacks(WinRateCallback)
            
            # Build and train algorithm
            algorithm = algo_config.build()
            
            # Training loop
            max_iterations = training_config.get("training", {}).get("max_iterations", 500) if training_config else 500
            best_reward = -np.inf
            best_win_rate = 0.0
            
            logger.info(f"Starting training for {max_iterations} iterations...")
            
            for i in range(max_iterations):
                result = algorithm.train()
                
                # Extract metrics
                episode_reward_mean = result.get("episode_reward_mean", 0.0)
                custom = result.get("custom_metrics", {})
                win_rate = custom.get("win_rate_mean", custom.get("win_rate_max", 0.0))
                
                if episode_reward_mean > best_reward:
                    best_reward = episode_reward_mean
                
                if win_rate > best_win_rate:
                    best_win_rate = win_rate
                
                # Log progress
                if (i + 1) % 25 == 0:
                    logger.info(f"Iteration {i+1}/{max_iterations}: reward={episode_reward_mean:.2f}, win_rate={win_rate:.2%}")
                    
                    # Checkpoint
                    checkpoint_path = algorithm.save(f"checkpoints/asx_stocks/asx_stocks/asx_stocks_checkpoint_{i+1}")
                
                # Early stopping if target achieved
                if win_rate >= 0.70 and i >= 100:
                    logger.info(f"Target win rate achieved: {win_rate:.2%}")
                    break
            
            # Final checkpoint
            final_checkpoint = algorithm.save(f"checkpoints/asx_stocks/asx_stocks/asx_stocks_final")
            
            training_metrics = {
                "episodes_trained": i + 1,
                "final_reward": episode_reward_mean,
                "best_reward": best_reward,
                "win_rate": best_win_rate,
                "checkpoint_path": final_checkpoint,
                "target_achieved": best_win_rate >= 0.70
            }
            
            # Save checkpoint metadata pointing to RLlib checkpoint
            self._save_model_checkpoint(training_metrics, final_checkpoint)
            
            self.training_results = training_metrics
            logger.info(f"Training completed. Final reward: {episode_reward_mean:.4f}, Win rate: {best_win_rate:.2%}")
            
            return training_metrics
            
        except Exception as e:
            logger.error(f"Training failed: {e}", exc_info=True)
            raise
    
    def _create_ppo_config_from_yaml(self, config: Dict[str, Any]) -> PPOConfig:
        """Create PPO config from YAML training config"""
        env_config = config.get("environment", {})
        training_config = config.get("training", {})
        resources_config = config.get("resources", {})
        
        # Create environment config dict (not instantiate env)
        env_config_dict = {
            "num_assets": len(self.config.symbols),
            "symbols": self.config.symbols,
            "initial_cash": env_config.get("initial_cash", 1000000),
            "episode_length": env_config.get("episode_length", 252),
            "lookback_window_size": env_config.get("lookback_window_size", 30),
            "transaction_cost_rate": env_config.get("transaction_cost_rate", 0.001),
            "use_real_data": True,
            "market_data": self.load_real_data()[0],
            "reward_weights": env_config.get("reward_weights", {})
        }
        
        # Create PPO config with env class and config (not instance)
        algo_config = PPOConfig().environment(env=WealthArenaTradingEnv, env_config=env_config_dict)
        algo_config.training(
            lr=training_config.get("learning_rate", 3e-4),
            gamma=training_config.get("gamma", 0.99),
            lambda_=training_config.get("gae_lambda", 0.95),  # Changed from gae_lambda to lambda_
            clip_param=training_config.get("clip_param", 0.2),
            entropy_coeff=training_config.get("entropy_coeff", 0.01),
            vf_loss_coeff=training_config.get("vf_loss_coeff", 0.5),
            train_batch_size=training_config.get("train_batch_size", 4000),
            minibatch_size=training_config.get("sgd_minibatch_size", 128),  # Changed from sgd_minibatch_size to minibatch_size
            num_sgd_iter=training_config.get("num_sgd_iter", 10)
        )
        algo_config.env_runners(
            num_env_runners=resources_config.get("num_workers", 4),
            num_envs_per_env_runner=resources_config.get("num_envs_per_worker", 2),
            num_cpus_per_env_runner=resources_config.get("num_cpus_per_worker", 1)
        )
        algo_config.learners(
            num_learners=1,
            num_gpus_per_learner=resources_config.get("num_gpus", 0)
        )
        algo_config.callbacks(WinRateCallback)
        
        return algo_config
    
    def backtest_agent(self, test_data: pd.DataFrame = None) -> Dict[str, Any]:
        """Backtest the trained agent using real data"""
        logger.info("Starting ASX Stocks agent backtest...")
        
        # Load test data
        if test_data is None:
            train_data, val_data, test_data = self.load_real_data()
            if test_data is None or test_data.empty:
                logger.warning("No test data available, using synthetic data")
                test_data = self.generate_synthetic_data(500)
        
        # Simulate trading episodes
        portfolio_values = [self.config.initial_cash]
        positions_history = [{}]
        returns = []
        
        # Load trained model if available
        algorithm = None
        try:
            checkpoint_files = list(Path("checkpoints/asx_stocks/asx_stocks").glob("*checkpoint*"))
            if checkpoint_files:
                latest_checkpoint = max(checkpoint_files, key=lambda p: p.stat().st_mtime)
                algorithm = PPO.from_checkpoint(str(latest_checkpoint))
                logger.info(f"Loaded checkpoint: {latest_checkpoint}")
        except Exception as e:
            logger.warning(f"Could not load checkpoint: {e}, using random actions")
        
        # Create test environment
        env_config = {
            "num_assets": len(self.config.symbols),
            "symbols": self.config.symbols,
            "initial_cash": self.config.initial_cash,
            "episode_length": len(test_data),
            "lookback_window_size": self.config.lookback_window,
            "transaction_cost_rate": self.config.transaction_cost_rate,
            "use_real_data": True,
            "market_data": test_data
        }
        test_env = WealthArenaTradingEnv(config=env_config)
        
        # Run backtest episodes
        num_episodes = 20
        all_trades = []
        
        for episode in range(num_episodes):
            obs, info = test_env.reset()
            episode_trades = []
            
            for i in range(len(test_data)):
                if algorithm:
                    # Use trained model
                    action = algorithm.compute_single_action(obs)
                else:
                    # Random action fallback
                    action = test_env.action_space.sample()
                
                obs, reward, terminated, truncated, info = test_env.step(action)
                episode_trades.extend(info.get('trades', []))
                
                if terminated or truncated:
                    break
            
            all_trades.extend(episode_trades)
        
        # Calculate actual win rate from trades
        win_rate = self.calculate_win_rate(all_trades)
        
        # Get portfolio values from environment
        portfolio_values = test_env.portfolio_value_history if hasattr(test_env, 'portfolio_value_history') else [self.config.initial_cash]
        
        # Calculate returns
        returns = []
        for i in range(1, len(portfolio_values)):
            period_return = (portfolio_values[i] - portfolio_values[i-1]) / portfolio_values[i-1]
            returns.append(period_return)
        
        positions_history = [{}] * len(portfolio_values)
        
        # Get real benchmark data (ASX 200)
        logger.info("Fetching real ASX 200 benchmark data...")
        benchmark_returns = self.benchmark_fetcher.get_asx_benchmark()
        
        # Align benchmark with our returns - ensure same length
        if len(benchmark_returns) != len(returns):
            if len(benchmark_returns) > len(returns):
                # Take the last N days to match our returns length
                benchmark_returns = benchmark_returns.tail(len(returns))
            else:
                # If benchmark is shorter, pad with zeros or repeat last value
                benchmark_returns = benchmark_returns.reindex(
                    pd.date_range(start=benchmark_returns.index[0], periods=len(returns), freq='D'),
                    method='ffill'
                )
        
        # Ensure we have exactly the same length
        benchmark_returns = benchmark_returns.iloc[:len(returns)]
        
        # Convert to numpy array
        benchmark_returns = benchmark_returns.values
        
        # Evaluate performance
        evaluation_metrics = self.evaluator.evaluate_performance(
            portfolio_values=portfolio_values,
            returns=np.array(returns),
            positions_history=positions_history,
            benchmark_returns=benchmark_returns
        )
        
        self.evaluation_results = {
            "portfolio_values": portfolio_values,
            "returns": returns,
            "positions_history": positions_history,
            "benchmark_returns": benchmark_returns.tolist(),
            "metrics": evaluation_metrics
        }
        
        logger.info("Backtest completed successfully")
        return self.evaluation_results
    
    def generate_report(self) -> Dict[str, Any]:
        """Generate comprehensive evaluation report"""
        if not self.evaluation_results:
            return {}
        
        metrics = self.evaluation_results["metrics"]
        
        return {
            "agent_info": {
                "asset_type": "asx_stocks",
                "symbols": self.config.symbols[:10],  # Show first 10
                "num_stocks": len(self.config.symbols),
                "training_period": f"{self.config.start_date} to {self.config.end_date}"
            },
            "training_results": self.training_results,
            "evaluation_metrics": metrics,
            "performance_summary": {
                "total_return": f"{metrics['returns']['total_return']:.2%}",
                "annual_return": f"{metrics['returns']['annual_return']:.2%}",
                "volatility": f"{metrics['returns']['volatility']:.2%}",
                "sharpe_ratio": f"{metrics['risk']['sharpe_ratio']:.3f}",
                "max_drawdown": f"{metrics['risk']['max_drawdown']:.2%}",
                "win_rate": f"{metrics['trading']['win_rate']:.2%}",
                "profit_factor": f"{metrics['trading']['profit_factor']:.3f}"
            },
            "generated_at": datetime.now().isoformat()
        }
    
    def save_results(self, output_dir: str = "results/asx_stocks"):
        """Save training and evaluation results"""
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)
        
        report = self.generate_report()
        with open(output_path / "evaluation_report.json", "w") as f:
            json.dump(report, f, indent=2, default=str)
        
        logger.info(f"Results saved to {output_path}")
    
    def _save_model_checkpoint(self, training_metrics: Dict[str, Any], final_checkpoint_path: str):
        """Save model checkpoint metadata pointing to RLlib checkpoint directory"""
        
        # Create metadata with RLlib checkpoint path
        metadata = {
            "agent_type": "asx_stocks",
            "symbols": self.config.symbols,
            "num_assets": len(self.config.symbols),
            "training_period": f"{self.config.start_date} to {self.config.end_date}",
            "episode_length": self.config.episode_length,
            "lookback_window": self.config.lookback_window,
            "max_position_size": self.config.max_position_size,
            "transaction_cost_rate": self.config.transaction_cost_rate,
            "rllib_checkpoint_path": final_checkpoint_path,
            "checkpoint_type": "rllib"
        }
        
        # Save metadata JSON pointing to RLlib checkpoint
        checkpoint_dir = Path("checkpoints/asx_stocks")
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        
        metadata_file = checkpoint_dir / "latest_checkpoint.json"
        metadata_data = {
            "checkpoint_path": final_checkpoint_path,
            "checkpoint_type": "rllib",
            "metadata": metadata,
            "training_metrics": training_metrics
        }
        
        with open(metadata_file, "w") as f:
            json.dump(metadata_data, f, indent=2, default=str)
        
        logger.info(f"Checkpoint metadata saved: {metadata_file} pointing to RLlib checkpoint: {final_checkpoint_path}")
        return str(metadata_file)


def main():
    """Main function to train and evaluate ASX Stocks agent"""
    logging.basicConfig(level=logging.INFO)
    
    config = ASXStocksConfig(
        symbols=get_asx_200_symbols()[:30],  # Top 30 ASX stocks
        start_date="2015-01-01",
        end_date="2025-09-26"
    )
    
    trainer = ASXStocksTrainer(config)
    
    print("[STARTING] Training ASX Stocks Agent...")
    training_results = trainer.train_agent()
    print(f"[OK] Training completed. Final reward: {training_results['final_reward']:.4f}")
    
    print("\n[SUMMARY] Running Backtest...")
    evaluation_results = trainer.backtest_agent()
    print("[OK] Backtest completed")
    
    print("\n[BREAKDOWN] Generating Evaluation Report...")
    report = trainer.generate_report()
    
    print("\n" + "="*60)
    print("ASX STOCKS AGENT EVALUATION RESULTS")
    print("="*60)
    
    print(f"\n[SUMMARY] PERFORMANCE SUMMARY:")
    for key, value in report["performance_summary"].items():
        print(f"  {key.replace('_', ' ').title()}: {value}")
    
    if report["evaluation_metrics"]["benchmark"]:
        print(f"\n[BREAKDOWN] BENCHMARK COMPARISON:")
        benchmark = report["evaluation_metrics"]["benchmark"]
        print(f"  Excess Return: {benchmark['excess_return']:.2%}")
        print(f"  Alpha: {benchmark['alpha']:.2%}")
        print(f"  Beta: {benchmark['beta']:.3f}")
    
    trainer.save_results()
    print(f"\n[SAVE] Results saved to results/asx_stocks/")
    
    return trainer, report


if __name__ == "__main__":
    trainer, report = main()
