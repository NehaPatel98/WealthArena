...
class WealthArenaMultiAgentEnv(MultiAgentEnv):
    ...
    def _setup_spaces(self):
        ...
        # Market observation from get_market_observation():
        #   - OHLCV: 5 * num_assets
        #   - Technical indicators: 8 * num_assets (sma_short, sma_long, rsi, bb_upper, bb_lower, macd, macd_signal, macd_histogram)
        #   - Microstructure: 2 * num_assets (bid_prices, ask_prices)
        #   Total market: 15 * num_assets
        # Portfolio observation: num_assets (position weights) + 2 (cash_ratio, value_ratio) = num_assets + 2
        # Risk observation: 4 (volatility, max_drawdown, sharpe_ratio, correlation)
        # Time observation: 1 (current_step)
        # Total: 15 * num_assets + num_assets + 2 + 4 + 1 = 16 * num_assets + 7
        obs_dim = (
            16 * self.num_assets +  # Market data (15) + portfolio weights (1)
            7                         # Portfolio ratios (2) + risk metrics (4) + time (1)
        )
        
        # Create single Box space for homogeneous agents
        ...
        # For MultiAgentEnv in RLlib, define spaces as dict mapping agent IDs to spaces
        # This allows RLlib to iterate over agents while keeping single space definition
        self.observation_space = gym.spaces.Dict({
            agent_id: obs_space for agent_id in self.agent_ids
        })
        ...
        # Also store the single space for easy access
...
if __name__ == "__main__":
    ...
    # Test a few steps
    for i in range(5):
        # Use single action space for sampling
