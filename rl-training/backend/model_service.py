    ...
    def _discover_models(self):
        ...
        for model_type in model_types:
            ...
            if model_path.exists():
                ...
                logger.info(f"[OK] Found model: {model_type}")
...
def get_model_service(model_dir: Path = None) -> RLModelService:
    """
    Get or create model service singleton
    
    Args:
        model_dir: Optional model directory path. If provided and different from current,
                  updates the model directory and triggers model discovery.
    
    Returns:
        RLModelService: The model service instance
    """
    global _model_service
    if _model_service is None:
        if model_dir:
            _model_service = RLModelService(model_dir=model_dir)
        else:
            _model_service = RLModelService()
    elif model_dir and _model_service.model_dir != model_dir:
        # Update model directory and trigger discovery
        _model_service.model_dir = model_dir
        _model_service._discover_models()
