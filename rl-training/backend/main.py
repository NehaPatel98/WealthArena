...
async def get_top_setups(request: TopSetupsRequest):
    ...
    try:
        ...
        logger.info(f"[OK] Returning {len(top_setups)} top setups for {request.asset_type}")
...
if __name__ == "__main__":
    ...
    logger.info("[STARTING] Starting WealthArena Backend API...")
    logger.info(f"[PATH] Data Directory: {DATA_DIR}")
    logger.info(f"[SUMMARY] Features Directory: {FEATURES_DIR}")
