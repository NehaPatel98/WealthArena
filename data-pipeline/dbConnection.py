...
def _connect_from_parts():
    ...
    placeholder_patterns = [
        # For SQL_SERVER: flag if starts with "your-" or "your_" or is "localhost"
        ("SQL_SERVER", server_raw, lambda v: v.lower().startswith("your-") or 
                                                v.lower().startswith("your_") or 
                                                v.lower() == "localhost"),
    ...
    if placeholder_found:
        error_msg = (
            ...
            f"  2. Replace ALL placeholder values with your actual Azure SQL credentials:\n"
            f"     - SQL_SERVER: Your Azure SQL server hostname (e.g., myserver.database.windows.net)\n"
            f"     - SQL_DB: Your database name\n"
            f"     - SQL_UID: Your SQL username\n"
            f"     - SQL_PWD: Your SQL password\n\n"
            f"  To obtain these credentials:\n"
            f"     - Azure Portal → SQL Databases → Your Database → Overview\n"
            f"     - Azure Portal → SQL Servers → Your Server → Connection Strings\n\n"
            f"  3. Save the file and re-run Phase 3\n\n"
            f"For detailed instructions, see:\n"
            f"  - data-pipeline/SETUP_INSTRUCTIONS.md (Section 4: Configuration Files)\n"
            f"  - data-pipeline/README_PHASE2.md (Step 1: Configure Environment Files)\n"
            f"{'='*70}\n"
        )
    ...
    try:
        ...
        if "IM002" in str(e) or "Data source name not found" in str(e):
            ...
            print(error_msg, file=sys.stderr)
            raise RuntimeError("ODBC Driver 18 for SQL Server is not installed. See error message above for installation instructions.") from e
        raise
